...
export function PromotionForm() {
  ...
  const [gananciaPromocion, setGananciaPromocion] = useState<number>(0)
  ...
  useEffect(() => {
    if (selectedProductId) {
      ...
      if (foundProduct) {
        ...
        // Calcular margen de ganancia actual
        const margen = (((foundProduct.precio - foundProduct.costo) / foundProduct.costo) * 100).toFixed(2)
        setMargenGanancia(Number.parseFloat(margen))

        // Calcular ganancia con promoción
        const gananciaPromo = (
          ((Number.parseFloat(precioConDescuento) - foundProduct.costo) / foundProduct.costo) *
          100
        ).toFixed(2)
        setGananciaPromocion(Number.parseFloat(gananciaPromo))
      }
    }
  }, [selectedProductId])
  ...
  // Calcular precio promoción cuando cambia el descuento
  useEffect(() => {
    if (product && descuento) {
      ...
      if (!isNaN(descuentoNum)) {
        ...
        if (Number.parseFloat(precioCalculado) < product.costo) {
          setErrorPrecio(true)
        } else {
          setErrorPrecio(false)
          // Calcular ganancia con promoción
          const gananciaPromo = (((Number.parseFloat(precioCalculado) - product.costo) / product.costo) * 100).toFixed(
            2,
          )
          setGananciaPromocion(Number.parseFloat(gananciaPromo))
        }
  ...
  // Calcular descuento cuando cambia el precio promoción
  useEffect(() => {
    if (product && precioPromocion && !isNaN(Number.parseFloat(precioPromocion))) {
      ...
      // Verificar si el precio es menor al costo
      if (precioNum < product.costo) {
        setErrorPrecio(true)
      } else {
        setErrorPrecio(false)
        // Calcular ganancia con promoción
        const gananciaPromo = (((precioNum - product.costo) / product.costo) * 100).toFixed(2)
        setGananciaPromocion(Number.parseFloat(gananciaPromo))
      }
    }
  }, [precioPromocion, product])
